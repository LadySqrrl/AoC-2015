...
{
	string path;
	int x, y, currX, currY, count, uniqueStops;
	bool repeats;
	vector<int> coords;
	vector<int> stops;
	coords.push_back(0);
	...
	for (int i = 0; i < (coords.size() - 2); i += 2) {
		...
		for (int step = (i + 2); step < coords.size(); step += 2) {
			if (currX < 9999) {
	...
	//move unique coords to new vector
	for (int i = 0; i < coords.size(); i++) {
		if (coords[i] < 9999) {
			stops.push_back(coords[i]);
		}
	}

	//count up the unique stops made (total coordinates in new vector / 2)
	uniqueStops = stops.size() / 2;
